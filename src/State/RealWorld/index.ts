...
class VendingMachineContext {
    ...
    public hasStockOf(product: Product): boolean {
        return this.inventory.items.some(
            (i) => i.product.name === product.name && i.items > 0
        );
    }
    ...
    public dispenseProduct(product: Product) {
        ...
        if (!this.hasStockOf(product)) {
            throw new Error(
                `No ${product.name} products left, select another one`
            );
        }
        ...
        const restOfInventory = this.inventory.items.filter(
            (i) => i.product.name !== product.name
        );
        this.inventory.items = [...restOfInventory, newInventoryItem];
        console.log(
            `Product ${product.name} dispensed. Inventory is now:`,
...
/**
 *We will implement only 3 states. States are only responsible for the
...
/**
 *The client code should handle edge cases and errors, in this case, only
